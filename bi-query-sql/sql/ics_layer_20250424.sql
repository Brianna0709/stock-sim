...
SELECT 
    ...
    ROUND(
        COUNT(DISTINCT s.host_id) * 100.0 / 
        NULLIF(COUNT(DISTINCT h.phx_user_id), 0), 
        2
    ) AS open_rate_pct
FROM ba_phx.phx_dim_supply_host h
LEFT JOIN open_smart_reply_hosts s 
    ON h.phx_user_id = s.host_id
WHERE h.dt = '20250424'
...
SELECT 
    ...
    ROUND(
        COUNT(DISTINCT s.host_id) * 100.0 / 
        NULLIF(COUNT(DISTINCT c.host_id), 0), 
        2
    )
...
SELECT 
    ...
    ROUND(
        COUNT(DISTINCT s.host_id) * 100.0 / 
        NULLIF(COUNT(DISTINCT a.host_id), 0), 
        2
    )
FROM ba_phx.phx_dim_supply_product_active_derive a
LEFT JOIN open_smart_reply_hosts s 
    ON a.host_id = s.host_id
WHERE a.dt = '20250424'
    AND a.product_operation_type = 1
