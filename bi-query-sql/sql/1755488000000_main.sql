select
  ...
  sum(im_denom) as total_im_denom,
  sum(im_numer) as total_im_numer,
  sum(call_denom) as total_call_denom,
  sum(call_numer) as total_call_numer,
  round((sum(im_numer)+sum(call_numer))/(sum(im_denom)+sum(call_denom)),4) as overall_1min_reply_rate,
  round(sum(im_numer)/sum(im_denom),4) as im_only_1min_rate,
  round(sum(call_numer)/sum(call_denom),4) as call_only_answer_rate
from (
  select
    coalesce(t1.host_id, t3.host_id) as host_id,
    coalesce(t1.im_numer, 0) as im_numer,
    coalesce(t1.im_denom, 0) as im_denom,
    coalesce(t3.call_numer, 0) as call_numer,
    coalesce(t3.call_denom, 0) as call_denom
  from (
    select
