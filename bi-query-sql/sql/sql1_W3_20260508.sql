...
unsolved_stats AS ( 
    ...
    FROM
        origindb_ss.hotel_ia_phx_user__phx_auto_reply_msg_survey
    WHERE
        dt =  '20260508' 
        AND gmt_create >= '2026-05-02 00:00:00'
        AND gmt_create < '2026-05-08 00:00:00'
        AND msg_type = 2
),

intelligent_reply_stats AS (
    SELECT
        COUNT(DISTINCT msg_id) AS total_replies
    FROM
        ba_phx.phx_mdw_detail_message_sync
    WHERE
        dt BETWEEN '20260502'  AND '20260508'
        AND gmt_create >= '2026-05-02 00:00:00'
        AND gmt_create < '2026-05-08 00:00:00'
...
SELECT
    ...
    CASE 
        ...
        THEN ROUND(
            ...
            (SELECT total_replies FROM intelligent_reply_stats), 
            4
        )
        ELSE 0 
