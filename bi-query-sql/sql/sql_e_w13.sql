-- SQL-E: 智能IM房东反馈错误率（SQL2口径） (W13: 0703-0709)
WITH host_survey AS (
    SELECT COUNT(id) AS host_survey
    FROM ba_phx.bas_phx_ai_reply_msg_host_survey
    WHERE gmt_create BETWEEN '2026-07-03 00:00:00' AND '2026-07-09 23:59:59'
),
ai_reply AS (
    SELECT COUNT(reply_id) AS reply_count
    FROM log.phx_hsop_osv_ai_reply_log
    WHERE dt BETWEEN '20260703' AND '20260709'
      AND _mt_datetime BETWEEN '2026-07-03 00:00:00' AND '2026-07-09 23:59:59'
