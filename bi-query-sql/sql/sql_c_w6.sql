SELECT
    s.dt AS stat_date,
    COUNT(DISTINCT s.host_id) AS total_online_active_hosts,
    COUNT(DISTINCT CASE WHEN s.ics_switch = 0 AND DATE_FORMAT(s.gmt_modify, 'yyyyMMdd') = s.dt THEN s.host_id END) AS daily_close_hosts,
    COUNT(DISTINCT CASE WHEN s.ics_switch = 0 THEN s.host_id END) AS daily_closed_status_hosts,
    ROUND(
