-- SQL-E: 智能IM房东反馈错误率 (W11: 2026-06-20 ~ 2026-06-26)
-- gmt_create/_mt_datetime含首尾完整时间
-- queue: root.zw06_2.hadoop-phx.query

WITH host_survey AS (
    SELECT COUNT(id) AS host_survey
    FROM ba_phx.bas_phx_ai_reply_msg_host_survey
    WHERE gmt_create BETWEEN '2026-06-20 00:00:00' AND '2026-06-26 23:59:59'
),
ai_reply AS (
    SELECT COUNT(reply_id) AS reply_count
    FROM log.phx_hsop_osv_ai_reply_log
    WHERE dt BETWEEN '20260620' AND '20260626'
      AND _mt_datetime BETWEEN '2026-06-20 00:00:00' AND '2026-06-26 23:59:59'
