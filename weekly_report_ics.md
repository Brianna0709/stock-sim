...
| 指标 | W2(0425-0501) | W3(0502-0508) | W4(0509-0514) | W5(0515-0521) | W6(0522-0528) | W7(0529-0604) | W8(0605-0611) | W9(0612-0618) | W10(0613-0619) | W11(0619-0625) | W12(0626-0702) | W13(0710-0716) | W14(0717-0723) | W15(0725-0731) | W16(0801-0807) | W17(0808-0814) | W18(0815-0821) | W19(0822-0828) | W20(0829-0904) | W21(0905-0911) |
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 智能回复使用率-有在线房源房东 | 49.86% | 51.37% | 52.25% | 29.71% | 30.02% | 30.98% | 31.04% | 31.61% | 32.2% | 31.77% | 33.88% | 36.47% | 37.69% | 38.24% | 41.22% | 41.17% | 40.11% | 35.74% | 31.51% | 31.08% | 31.67% |
| 智能客服关闭率-房东(操作关闭率日均) | 0.30% | 0.08% | 0.03% | 0.03% | 0.03% | 0.03% | 0.03% | 0.035% | 0.04% | 0.04% | 0.05% | 0.05% | 0.06% | 0.06% | 0.09% | 0.053% | 0.035% | 0.025% | 0.03% | 0.03% | 0.035% |
| 智能IM房客点击未解决率(SQL1) | 0.71% | 0.72% | 0.66% | 0.70% | ⚠️待确认 | 0.68% | ⚠️待跑(魔数) | 0.6962% | 0.7046% | 0.6753% | 0.6605% | ⚠️待确认 | ⚠️分区无数据(魔数) | 0.63% | ⚠️待确认(2.27%) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) |
| 智能IM房东反馈错误率(SQL2) | 0.19% | 0.17% | 0.16% | 0.16% | 0.16% | 0.17% | 0.17% | 0.18% | 0.17% | 0.17% | 0.19% | 0.24% | 0.27% | 0.26% | 0.27% | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) | ⚠️无权限(魔数) |
| 智能回复消息覆盖率 | 22.92% | 22.45% | 22.30% | 22.36% | 22.50% | 22.70% | 22.77% | 23.00% | 23.04% | 23.35% | 23.52% | 23.98% | 23.63% | 23.78% | 22.83% | 23.73% | 23.28% | 23.06% | 17.64% | 22.98% | 23.25% |
| 智能回复会话覆盖率 | 71.45% | 71.85% | 72.78% | 72.40% | 72.98% | 73.29% | 73.78% | 74.02% | 74.01% | 74.79% | 74.98% | 75.26% | 75.59% | 75.73% | 72.53% | 75.07% | 75.46% | 75.63% | 61.09% | 75.39% | 75.56% |
...
-- 周维度：如 '20260905' AND '20260911'
-- 月维度：如 '20260901' AND '20260930'
...
daily AS (
    ...
    WHERE dt BETWEEN '20260905' AND '20260911'
...
> ⚠️ 注意：dt BETWEEN 用**周度范围**（如0905-0911），维度表快照用当周最后一天（若当天数据未就绪则用最新可用日）

```sql
-- 修改 dt BETWEEN（ai_reply_log）和 dt（维度表快照）即可复用
-- 周维度：ai_reply_log 用统计周范围，维度表用当周最后一天（若当天数据未就绪则用最新可用日）
-- 示例：W21 用 20260905-20260911，维度表快照用 20260910（20260911 数据未就绪）
...
online_hosts_stats AS (
    ...
    WHERE c.dt = '20260910'  -- 当周最后一天快照（若0911未就绪则用最新可用）
...
WHERE s.dt BETWEEN '20260905' AND '20260911'  -- 改时间范围
  AND s.status = 1
...
-- dt IN 当周所有实际日分区（如 '20260905' 至 '20260911'）
-- gmt_create 范围 = 实际统计周（如 W21: 2026-09-05 ~ 2026-09-11）
...
WITH unsolved_stats AS (
    ...
    WHERE dt IN ('20260905','20260906','20260907','20260908','20260909','20260910','20260911')
      AND gmt_create >= '2026-09-05 00:00:00'
      AND gmt_create <  '2026-09-12 00:00:00'
...
-- W21 示例：2026-09-05 00:00:00 ~ 2026-09-11 23:59:59

WITH host_survey AS (
    SELECT COUNT(id) AS host_survey
    FROM ba_phx.bas_phx_ai_reply_msg_host_survey
    WHERE gmt_create BETWEEN '2026-09-05 00:00:00' AND '2026-09-11 23:59:59'
),
ai_reply AS (
    SELECT COUNT(reply_id) AS reply_count
    FROM log.phx_hsop_osv_ai_reply_log
    WHERE dt BETWEEN '20260905' AND '20260911'
      AND _mt_datetime BETWEEN '2026-09-05 00:00:00' AND '2026-09-11 23:59:59'
      AND ai_msg_recommend_strategy > 0
)
SELECT
...
1. **日期格式**：SQL 里日期用 `'YYYYMMDD'` 格式（如 `'20260911'`）
...
8. **W22(0912-0918) 备注**：使用率分母快照用 20260917（0918 分区未就绪）；关闭率为 0912-0917 六天日均（0918 分区缺失）；SQL-D/E 因 CLI 无权限未跑，待魔数界面执行；指定队列 root.zw06_2.hadoop-phx.query 无成员权限，已改用默认队列